...
class ImageToolComponent extends Component{
	 ...
	 *
	 * Options:
	 * - 'file' Input file (path or gd resource)
	 * - 'path' Output path. If not specified, gd resource is returned
	 * - 'remove' Remove before save actual image
	 * - 'width/height' Size of image to output
	 *
	 * @return mixed boolean
	 */
	public function _upload($file, $remove = null, $width = 200, $heigh = 200){
		$ext = self::getExtension($file['name']);
		$new = Text::uuid().'.'.$ext;
		$options = [
			'input' => $file['tmp_name'],
			'output' => WWW_ROOT . "img/thumbs/{$new}",
			'width' => $width,
			'height' => $heigh,
			//'quality' => 100,
			'mode' => 'fit',
		...
		if(!is_null($remove)){
			$this->_remove(WWW_ROOT . "img/{$remove}");
		}
